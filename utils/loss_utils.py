...
def ssim_multispectral(img1, img2, window_size=11, size_average=True):
    print('img1.shape', img1.shape, 'img2.shape', img2.shape)
    channels = img1.size(-3)
    print('img1.size(0 -1 -2 -3', img1.size(0), img1.size(-1), img1.size(-2), img1.size(-3))
    ...
    for channel in range(channels):
        res += _ssim(img1, img2, window, window_size, channel, size_average)

    return res

def _ssim(img1, img2, window, window_size, channel, size_average=True):
    img2 = img2.type_as(img1)
    window = window.to(img1.device).type_as(img1) # ensure window is on the same device and type as img1
