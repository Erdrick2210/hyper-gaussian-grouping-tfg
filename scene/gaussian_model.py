...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.num_objects = 24 # 8, 16, 24, 32, 38 maximum
        # To be changed in:
    ...
    def training_setup(self, training_args):
        ...
        self.model = training_args.Model(training_args.dim_embedding, training_args.channels)
        self.model.cuda()
        ...
        l = [
            ...
            {'params': self.model.parameters(), 'lr': training_args.model_lr, "name": "model"},
        ]
    ...
    def finetune_setup(self, training_args, mask3d):
        ...
        # per si cas
        hook_model = self.model.register_hook(mask_hook2)

        # self._objects_dc.requires_grad = False  # perque estava descomentat ?
        ...
        l = [
            ...
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
            {'params': [self._objects_dc], 'lr': training_args.objects_dc_lr, "name": "obj_dc"},
        ]
    ...
    def construct_list_of_attributes_embedding(self):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        # All channels except the 3 DC
        for i in range(self._features_dc.shape[1]*self._features_dc.shape[2]):
            l.append('f_dc_{}'.format(i))
    ...
    def save_ply(self, path):
        ...
        print('GaussianModel.save_ply : f_dc', f_dc.shape, 'f_rest', f_rest.shape, 'obj_dc', obj_dc.shape)
    ...
    # save_ply but replacing the sh with first 3 channels from embedding
    def save_ply_embedding(self, path):
        mkdir_p(os.path.dirname(path))

        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        f_dc = self._objects_dc.detach().transpose(1, 2).flatten(start_dim=1)[:,:3].contiguous().cpu().numpy()
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
        ...
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes_embedding()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, opacities, scale, rotation, obj_dc), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    
    # pca to pass from dim_embedding to 3 dimensions to substitute sh (f_dc)
    '''
    def save_ply_embedding(self, path):
        mkdir_p(os.path.dirname(path))
        ...
        f_dc = pca.fit_transform(obj_dc).astype(np.float32)
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
    ...
    # t-sne to pass from dim_embedding to 3 dimensions to substitute sh (f_dc), VERY SLOW
    '''
    def save_ply_embedding(self, path):
        mkdir_p(os.path.dirname(path))
        ...
        if obj_dc.shape[1] > 3:
            tsne = TSNE(n_components=3, random_state=42, perplexity=min(30, obj_dc.shape[0] - 1))
            obj_dc_reduced = tsne.fit_transform(obj_dc)
            print('obj_dc after t-SNE', obj_dc_reduced.shape)
        else:
            obj_dc_reduced = obj_dc
            print('obj_dc already has 3 or less dimensions')

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, obj_dc_reduced, f_rest, opacities, scale, rotation, obj_dc), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    '''
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            if group['name'] != 'model':
                len_group = len(group["params"])
                assert len_group == 1, print('len(group["params"])={}'.format(len_group))
