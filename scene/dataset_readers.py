...
def readColmapCamerasMultispectral(cam_extrinsics, cam_intrinsics, images_folder, objects_folder):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        sys.stdout.write('\r')
        # the exact output you're looking for:
        ...
        image_path = os.path.join(images_folder, os.path.basename(extr.name))
        image_name = os.path.basename(image_path).split(".")[0]
        image_path = os.path.join(images_folder, image_name + '.npy')
        print(' image_path', image_path, 'image_name', image_name, 'extr.name', extr.name, ' ')
...
def readColmapSceneInfo(path, images, eval, object_path, llffhold=8, n_views=100, random_init=False, train_split=False):
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse", "images.bin")  # "sparse/0"
        cameras_intrinsic_file = os.path.join(path, "sparse", "cameras.bin") # "sparse/0"
    ...
    cam_infos_unsorted = readColmapCamerasMultispectral(
        cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,
        images_folder=os.path.join(path, reading_dir),
        objects_folder=os.path.join(path, object_dir))

    cam_infos = sorted(cam_infos_unsorted.copy(), key=lambda x: x.image_name)
    print('cam_infos.image_name', [cam_info.image_name for cam_info in cam_infos])
    ...
    if eval:
        ...
        print('train cameras', sorted([cam_info.image_name for cam_info in train_cam_infos]))
        print('test cameras', sorted([cam_info.image_name for cam_info in test_cam_infos]))
    ...
    if random_init:
        ...
        ply_path = os.path.join(path, "sparse/points3D.ply")  # "sparse/0"
        bin_path = os.path.join(path, "sparse/points3D.bin")  # "sparse/0"
        txt_path = os.path.join(path, "sparse/points3D.txt")  # "sparse/0"
